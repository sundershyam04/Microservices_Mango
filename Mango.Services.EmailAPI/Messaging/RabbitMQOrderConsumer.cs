...
namespace Mango.Services.EmailAPI.Messaging
{
    public class RabbitMQOrderConsumer : BackgroundService
    {
        ...
        protected override async Task<Task> ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            var queueDeclareOk = await _channel.QueueDeclareAsync("email-order-queue");
            ...
            consumer.ReceivedAsync += async(ch, ea) =>
            {
                ...
                var orderMessage = JsonConvert.DeserializeObject<OrderConfirmationMessage>(content);
                HandleMessage(orderMessage).GetAwaiter().GetResult();
        ...
        private async Task HandleMessage(OrderConfirmationMessage order)
        {
            await _emailService.EmailAndLogConfirmedOrder(order);
        }
    }
}
